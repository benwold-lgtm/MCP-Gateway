...
def test_console_is_exactly_what_a_password_session_reaches():
    ...
    (`devices:read`/`devices:write`), `/metrics/summary` (`metrics:read`), and the MCP
    invocation path (`tools:call`)."""
    assert scopes_for_role("console") == frozenset(
        {SCOPE_DEVICES_READ, SCOPE_DEVICES_WRITE, SCOPE_METRICS_READ, SCOPE_TOOLS_CALL}
    )
