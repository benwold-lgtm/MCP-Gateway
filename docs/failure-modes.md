...
| D10 | A device whose pod never spawns reports `reachable: true` forever | 🟡 | `spawn_error` is set while `reachable` reads `true`; `GET /devices/{h}/diagnostics` shows `last_check` absent or ageing without bound (observed growing 150s → 190s over a 40s wait) | The fleet listing and the UI show a device as reachable that has never once been contacted — the value is a dataclass default, not a measurement | **None — open ([F-66](findings-register.md))**. The health loop only iterates a worker's *assigned* devices (`worker/health.py`, `run_forever(assigned)`), and a device that never spawns is never assigned, so nothing ever writes `reachable` | Trust `spawn_error` and `last_check`, not `reachable`, for a device that has never come up |
