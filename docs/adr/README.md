...
| [0012](0012-federation-credential-model.md) | Credential model for BFF provider federation — preserve per-user relay, BFF audit first | Proposed — **the only record still open.** Its per-user-relay decision is validated by [0017](0017-provider-authority-is-delegated.md) §7; what remains is one sequence gated on a provider registry that does not exist yet (`gateway_url` is still singular). See its *Scope after the 2026-08-21 review* |
