...
| 5 | [0022](0022-agent-initiated-device-writes-are-plan-bound.md) | **In progress** (2026-08-25). Reuses ADR-0018 §6's canonicalization/digest utility, per the ADR's own "no new digest mechanism, no new canonicalization rules." Slices 1–3 of 5 built: the `PendingProposalStore`/`WritePlannedGrantStore` layer (`device_mcp_gateway/write_planned.py`), the `devices:write-planned` scope (deliberately never in `ROLE_SCOPES`), and the Propose route (`POST /v1/devices/plans`). Remaining: Review (approve a proposal, mint the grant) and Apply (redeem the grant, run the real registration/reconfiguration validation, write) |
