...
| 4 | [0018](0018-device-credentials-by-reference.md) §6: the plan digest | The critical path. Both restore and [0022](0022-agent-initiated-device-writes-are-plan-bound.md) wait on it. **Slice 1 built** (2026-08-25): the general-purpose canonicalization/digest utility and the HMAC-signed plan token settling the format-vs-validity-window conflict — `device_mcp_gateway/shared/canonical_json.py` and `shared/plan_token.py`. Not yet built: the route split, the contract wiring (`plan_digest`/`plan_token` on the dry-run response and apply request, both audit records), `ERR_PLAN_STALE`, and the BFF/web removal of `provider:credentials` and single-use |
