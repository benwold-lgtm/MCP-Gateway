...
| 4 | [0018](0018-device-credentials-by-reference.md) §6: the plan digest | The critical path — [0022](0022-agent-initiated-device-writes-are-plan-bound.md) waits on it. **Slices 1–3 built** (2026-08-25): the canonicalization/digest utility, the HMAC-signed plan token, the `preview`/`apply` route split, and the full contract wiring (`plan_digest`/`plan_token`, both audit records, `ERR_PLAN_STALE`) are all in `device_mcp_gateway/api/backup.py` + `shared/canonical_json.py` + `shared/plan_token.py`. **Restore's own dependency on this item is now done.** Not yet built: slice 4, the BFF/web removal of `provider:credentials` and single-use (UI repository) — [0022](0022-agent-initiated-device-writes-are-plan-bound.md) does not need slice 4, only 1–3, so it can start once this lands |
