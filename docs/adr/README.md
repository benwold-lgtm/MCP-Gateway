...
| 5 | [0022](0022-agent-initiated-device-writes-are-plan-bound.md) | **In progress** (2026-08-25). Reuses ADR-0018 §6's canonicalization/digest utility, per the ADR's own "no new digest mechanism, no new canonicalization rules." Slices 1–4 of 5 built: the `PendingProposalStore`/`WritePlannedGrantStore` layer, the `devices:write-planned` scope (deliberately never in `ROLE_SCOPES`), Propose (`POST /v1/devices/plans`), and Review (`GET .../plans/{id}` + `POST .../plans/{id}/approve`, both gated by `devices:write` rather than a new scope — approving a device-registry change is squarely inside what that scope already means). Remaining: Apply — redeem the grant, run the real registration/reconfiguration validation (the SSRF guard included) against the extracted `register_device`/`update_device` logic, write |
